...
private:
    ...
    void imp(TreeNode<T>* node, int& n, std::vector<T>& result);
};

template <class T>
void BST<T>::imp(int n) {
    std::vector<T> result;
    imp(this->root, n, result);
...
template <class T>
void BST<T>::imp(TreeNode<T>* node, int& n, std::vector<T>& result) {
    if (node == nullptr || n == 0) {
        return;
    }
    imp(node->getRight(), n, result);
    if (n > 0) {
        result.push_back(node->getInfo());
        n--;
    }
    imp(node->getLeft(), n, result);
}
...
void BST<T>::levelByLevel(TreeNode<T>* root) const {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        std::cout << current->getInfo().first << " ";
