...
class Graph {
    ...
    std::vector <std::pair<std::string,int>> incidentesCount;
...
public:
    ...
    std::vector <std::pair<std::string,int>> getIncidentesCount();
    
    template <class Vn, class En>
    friend std::ostream & operator <<(std::ostream &, const Graph<Vn,En> &);
    
    const std::vector<Vertex<V, E> *>& getVertices() const { return nodes; }
...
{
    for (auto v : nodes) {
        std::string IP = v->getInfo();
        int count =v->getIncidentesEntrada();
        incidentesCount.push_back(std::make_pair(IP, count));
    }
}

template <class V, class E>
std::vector <std::pair<std::string,int>> Graph<V,E>::getIncidentesCount(){
    return incidentesCount;
}
